...
- **Action:** Run `python3 ./.agent/scripts/quotestimator.py --summarize --mission {{MISSION_ID}} >> ./.agent-missions/{{MISSION_ID}}/FINAL_REVIEW.md`
- **Action:** Run `./.agent/scripts/archive-mission.sh {{MISSION_ID}} "closed-approved"`.
- **Action:** Reset `.agent/state/context.json`: `active_feature` → `"None"`; update `last_updated`. If any cross-cutting architectural decisions were made, update `docs/architecture/decisions.md`.
