...
#
# Creates ./.agent-missions/mission-YYYYMMDD-HHMMSS/, copies the MISSION_STATE.md
...
TEMPLATE="$ROOT_DIR/.agent/templates/MISSION_STATE.md"

if [[ ! -f "$TEMPLATE" ]]; then
  echo "error: MISSION_STATE.md template not found at $TEMPLATE" >&2
  exit 1
fi
...
# Instantiate MISSION_STATE.md from the template with placeholders filled in.
...
sed \
  ...
  "$TEMPLATE" > "$MISSION_PATH/MISSION_STATE.md"
